...
{
    public:
    YOLO_Interface(std::string filenameconfig, std::vector<std::string> class_filter);
    ~YOLO_Interface();
    void Detect(cv:: Mat img, std::vector<Object>& objs);
	...
	// "person", "car", "bird", "truck", "bench", "motorbike",
	// "bicycle", "bus", "dog", "cat", "face",
	// "plate", "clothes", "tree", "excavator", "dump_truck",
	// "concrete_mixer_truck", "van", "container_truck", "bulldozer", "vehicle",
	// "boat", "roller"};
    ...
    private:
    struct YOLOConfig {
        std::string detector_engine_file_path="yolov8n_new.bin";
        float score_thres_=0.5;
        float iou_thres_=0.4;
        int topk_=100;
        int num_labels_=80;
    } ;
    bool getYOLOconfig(std::string fileName);
